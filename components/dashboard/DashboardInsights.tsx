...
export default function DashboardInsights({ carId, month, year }: DashboardInsightsProps) {
  const { data: insightsData, isLoading } = useActivityInsights(carId, month, year);
  ...
  if (isLoading) {
    return (
      <View className="mt-4 px-2 mb-6 animate-pulse">
        <View className="bg-[#E0FBFC] h-24 rounded-[24px] border border-[#B8F2F4]" />
      </View>
    );
  }
      ...
      <View className="bg-[#E0FBFC] p-5 rounded-[12px] border border-[#B8F2F4]">
        <View className="flex-row items-center gap-3">
          ...
          <View className="flex-1">
            <Text className="font-lexendBold text-[12px] text-[#404040]">Insights</Text>
          ...
          <View className="flex-row gap-2">
            ...
            >
              <Ionicons
                ...
                size={20}
                color="#00AEB5"
                opacity={insights.length <= 1 ? 0.3 : 1}
              />
            </TouchableOpacity>
            <TouchableOpacity 
              onPress={handleNext}
